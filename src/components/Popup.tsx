...
interface PopupProps {
  isPopupForNotVerifiedUsersOpen: boolean,
  setIsPopupForNotVerifiedUsersOpen: Dispatch<SetStateAction<boolean>>
}

const Popup: React.FunctionComponent<PopupProps> = ({isPopupForNotVerifiedUsersOpen, setIsPopupForNotVerifiedUsersOpen}): ReactElement => {

  return (
      <Dialog open={isPopupForNotVerifiedUsersOpen} onClose={() => setIsPopupForNotVerifiedUsersOpen(false)} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="bg-white rounded-[12px] w-full max-w-sm pt-[20px] pl-[30px] pr-[30px] pb-[15px] flex flex-col">
            <div className="flex flex-row-reverse">
              <Cross onClick={() => setIsPopupForNotVerifiedUsersOpen(false)} className="opacity-30 outline-none hover:scale-110 transition-transform duration-200"/>
            </div>
